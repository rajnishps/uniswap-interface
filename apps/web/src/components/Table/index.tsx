...
import {
  TableSizeProvider,
  useTableSize,
} from "components/Table/TableSizeProvider";
...
import {
  ...
  useSporeColors,
} from "ui/src/hooks/useSporeColors";
...
const TableRow = ({ row }: { row: Row<RowData> }) => {
  ...
  const cells = row
    .getVisibleCells()
    .map((cell: Cell<RowData, any>) => (
      <TableCell key={cell.id} cell={cell} colors={colors} />
    ));
...
export function Table<T extends RowData>({
  ...
  const onScrollButtonPress = useCallback(
    (direction: ScrollButtonProps["direction"]) => () => {
      ...
      const regularColumns = table
        .getAllColumns()
        .slice(numPinnedVisibleColumns);
    ...
    <TableSizeProvider value={tableSize}>
      <ScrollSync horizontal>
        ...
        >
          ...
          <ScrollSyncPane group="table-sync">
            <TableBodyContainer
              maxHeight={maxHeight ? maxHeight - headerHeight : "unset"}
            >
              <TableBody
                loading={loading}
                error={error}
