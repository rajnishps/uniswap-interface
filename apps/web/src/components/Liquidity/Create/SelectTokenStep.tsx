      ...
      <Flex gap="$spacing8">
        <Flex row gap={10} justifyContent="space-between">
          <Text variant="buttonLabel3">
            {feeTier.value.isDynamic
              ? t("common.dynamic")
              : formatPercent(feeTier.value.feeAmount / BIPS_BASE, 4)}
          </Text>
        ...
        >
          <Flex>
            ...
            {feeTier.selectionPercent &&
              feeTier.selectionPercent.greaterThan(0) && (
                <Text variant="body4" color="$neutral2">
                  {t("fee.tier.percent.select", {
                    percentage: formatPercent(
                      feeTier.selectionPercent.toSignificant(),
                      3
                    ),
                  })}
                </Text>
          ...
          {isLpIncentivesEnabled &&
            feeTier.boostedApr !== undefined &&
            feeTier.boostedApr > 0 && (
              <LpIncentivesAprDisplay
                lpIncentiveRewardApr={feeTier.boostedApr}
                isSmall
              />
            )}
        </Flex>
...
export function SelectTokensStep({
  ...
  setCurrencyInputs: Dispatch<
    SetStateAction<{ tokenA: Maybe<Currency>; tokenB: Maybe<Currency> }>
  >;
} & FlexProps) {
  ...
  const [showWrappedNativeWarning, setShowWrappedNativeWarning] =
    useState(false);
  ...
  const isUnsupportedTokenSelected =
    isUnsupportedChain(token0?.chainId, protocolVersion) ||
    isUnsupportedChain(token1?.chainId, protocolVersion);
  ...
  const handleCurrencySelect = useCallback(
    (currency: Currency) => {
      ...
      const otherInputState =
        currencySearchInputState === "tokenA" ? "tokenB" : "tokenA";
      const otherCurrency = currencyInputs[otherInputState];
      const wrappedCurrencyNew = currency.isNative
        ? currency.wrapped
        : currency;
      const wrappedCurrencyOther = otherCurrency?.isNative
  ...
  const feeTierHasLpRewards = useMemo(
    () =>
      Object.values(feeTierData).some(
        (tier) => tier.boostedApr && tier.boostedApr > 0
      ) && isLpIncentivesEnabled,
  ...
  const mostUsedFeeTier = useMemo(() => {
    if (hasExistingFeeTiers && Object.keys(feeTierData).length > 0) {
      return Object.values(feeTierData).reduce((highest, current) => {
        return current.percentage.greaterThan(highest.percentage)
          ? current
          : highest;
      });
    }
  ...
  const token0FoTError = hasLPFoTTransferError(
    token0CurrencyInfo,
    protocolVersion
  );
  const token1FoTError = hasLPFoTTransferError(
    token1CurrencyInfo,
    protocolVersion
  );
  ...
  const defaultFeeTiers = getDefaultFeeTiersWithData({
    chainId: token0?.chainId,
    feeTierData,
    protocolVersion,
  });
    ...
    <>
      {hook && (
        <HookModal
          ...
          onClearHook={() =>
            setPositionState((state) => ({ ...state, hook: undefined }))
          }
          onContinue={() => {
            ...
            <Flex gap="$spacing12">
              <Flex>
                ...
                <Text variant="body3" color="$neutral2">
                  {tokensLocked
                    ? t("position.migrate.liquidity")
                    : t("position.provide.liquidity")}
                </Text>
              </Flex>
              ...
              />
              {!hasError && protocolVersion === ProtocolVersion.V4 && (
                <AddHook />
              )}
            </Flex>
          </Flex>
                  ...
                  >
                    <Flex gap="$gap4" flex={1} minWidth={0}>
                      <Flex row gap="$gap8" alignItems="center">
                        ...
                        (mostUsedFeeTier &&
                          getFeeTierKey(
                            mostUsedFeeTier.fee.feeAmount,
                            mostUsedFeeTier.fee.isDynamic
                          )) ? (
                          <MouseoverTooltip
                            text={t("fee.tier.recommended.description")}
                          >
                            <Flex
                    ...
                    <Button
                      fill={false}
                      isDisabled={
                        !currencyInputs.tokenA || !currencyInputs.tokenB
                      }
                      size="xsmall"
  ...
  if (fotToken) {
    return (
      <ErrorCallout
        ...
        description={t("position.fot.warning", {
