...
export function PoolDetailsStatsButtons({
  ...
  const handleAddLiquidity = async () => {
    if (currency0 && currency1) {
      const currency0Address = currency0.isNative
        ? NATIVE_CHAIN_ID
        : currency0.address;
      const currency1Address = currency1.isNative
        ? NATIVE_CHAIN_ID
        : currency1.address;
      ...
      if (tokenId) {
        navigate(
          `/positions/${protocolVersion?.toLowerCase()}/${chainUrlParam}/${tokenId}`,
          {
  ...
  const [warningModalCurrencyInfo, setWarningModalCurrencyInfo] =
    useState<Maybe<CurrencyInfo>>();
  const onWarningCardCtaPressed = useCallback(
    ...
    <Flex flexDirection="column" gap="$gap24">
      <PoolButtonsWrapper isMobile={isMobile}>
        <Flex
          ...
          gap={
            isMobile || screenSizeLargerThanTablet ? "$spacing12" : "$spacing16"
          }
          width="100%"
        >
          <PoolButton
