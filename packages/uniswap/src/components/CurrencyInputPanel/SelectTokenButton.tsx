...
export const SelectTokenButton = memo(function _SelectTokenButton({
  ...
  const hoverStyle: {
    backgroundColor: ComponentProps<typeof Flex>["backgroundColor"];
  } = useMemo(
    () => ({
      backgroundColor: selectedCurrencyInfo
        ? "$surface1Hovered"
  ...
  return (
    <TouchableArea
      backgroundColor={
        selectedCurrencyInfo ? "$surface1" : validTokenColor ?? "$accent1"
      }
